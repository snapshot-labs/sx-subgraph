...
const MASTER_SPACE = Address.fromString(
  "0x7cC62f8E9BF2b44ce704D2cdCd4aa8021d5A6f4B"
);
const MASTER_SIMPLE_QUORUM_AVATAR = Address.fromString(
  "0x6F12C67cAd3e566B60A6AE0146761110F1Ea6Eb2"
);
...
export function handleProxyDeployed(event: ProxyDeployed): void {
  if (event.params.implementation.equals(MASTER_SPACE)) {
    ...
    event.params.implementation.equals(MASTER_SIMPLE_QUORUM_TIMELOCK)
  ) {
...
export function handleSpaceCreated(event: SpaceCreated): void {
  ...
  space.strategies = event.params.votingStrategies.map<Bytes>(
    (strategy) => strategy.addr
  );
  space.strategies_params = event.params.votingStrategies.map<string>(
    (strategy) => strategy.params.toHexString()
  );
  ...
  if (space.validation_strategy.equals(VOTING_POWER_VALIDATION_STRATEGY)) {
    let params = decodeProposalValidationParams(
      event.params.proposalValidationStrategy.params
    );

    if (params) {
      space.proposal_threshold = new BigDecimal(
        getProposalValidationThreshold(params)
      );
  ...
  space.authenticators = event.params.authenticators.map<Bytes>(
    (address) => address
  );
...
export function handleProposalExecuted(event: ProposalExecuted): void {
  let proposal = Proposal.load(
    `${event.address.toHexString()}/${event.params.proposalId}`
  );
  if (proposal == null) {
    return;
  }
  ...
  let executionStrategy = ExecutionStrategy.load(
    proposal.execution_strategy.toHexString()
  );
  ...
  if (executionStrategy !== null) {
    ...
    if (executionStrategy.type == "SimpleQuorumTimelock") {
      proposal.execution_time =
        ...
        executionStrategy.timelock_delay.toI32();
    }
  }
...
export function handleVoteCreated(event: VoteCast): void {
  ...
  let vote = new Vote(
    `${space.id}/${event.params.proposalId}/${event.params.voter.toHexString()}`
  );
...
export function handleMinVotingDurationUpdated(
...
export function handleMaxVotingDurationUpdated(
...
export function handleTimelockProposalExecuted(
  event: TimelockProposalExecuted
): void {
  let executionHash = ExecutionHash.load(
    event.params.executionPayloadHash.toHexString()
  );
